...
class MobileNetV2(nn.Module):
    def __init__(self, input_channel=3,  n_classes=10, width_multipler=1.0):
        ...
        self.cfgs = [
            # t, c, n, s
            [1,  16, 1, 1],
            [6,  24, 2, 1],  # 👈 原本是 stride 2，改成 stride 1 (保留解析度)
            [6,  32, 3, 2],
            [6,  64, 4, 2],
            [6,  96, 3, 1],
            [6, 160, 3, 2],
        ...
        self.features = [conv3x3(input_channel, first_channel, 1)]
        ...
        # 💡 [修復 1]：將 Pooling 註冊為正式的神經網路層，防止特徵提取器抓錯形狀
        self.pool = nn.AdaptiveAvgPool2d((1, 1))
        self.flatten = nn.Flatten(1)                    # 👈 將 Flatten 物件化
    ...
    def forward(self, x):
        ...
        # 💡 [修復 2]：使用標準層進行 Pooling，並正規化展平 (Flatten)
        x = self.pool(x)
        
        # x = torch.flatten(x, 1) # 將 (N, 1280, 1, 1) 展平為 (N, 1280) 的標準特徵向量
        x = self.flatten(x)   #
